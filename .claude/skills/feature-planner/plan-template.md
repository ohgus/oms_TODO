...
**🔍 Frontend Code Review** (프론트엔드 구현 Phase에서 필수):

- [ ] `/frontend-code-review [구현된 컴포넌트 경로]` 실행
- [ ] **가독성** 이슈 수정 (매직 넘버, 조건부 렌더링, 삼항 연산자)
- [ ] **예측 가능성** 이슈 수정 (반환 타입, 숨겨진 부작용, 네이밍)
- [ ] **응집도** 이슈 수정 (폼 응집도, 기능별 코드 조직)
- [ ] **결합도** 이슈 수정 (Props 드릴링, 상태 관리 범위)
- [ ] 리뷰 결과 Notes 섹션에 기록
...
# Frontend Code Review (프론트엔드 Phase에서 필수)
...
- [ ] `/frontend-code-review src/` 전체 코드 최종 리뷰 완료
- [ ] 모든 가독성 이슈 해결
- [ ] 모든 예측 가능성 이슈 해결
- [ ] 모든 응집도 이슈 해결
- [ ] 모든 결합도 이슈 해결
